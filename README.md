...
macOS 와 Windows 워크플로가 **둘 다 기존 `latest.json` 을 읽어 병합**합니다.
판 번호가 같으면 다른 플랫폼 엔트리를 그대로 두고 제 것만 갈아 끼웁니다.

**그래서 두 잡의 순서는 상관없습니다.** `needs:` 로 줄을 세울 필요가 없습니다.
판 번호가 다르면 이어받지 않습니다 - 옛 판의 자산을 새 안내문이 가리키면 안 됩니다.
